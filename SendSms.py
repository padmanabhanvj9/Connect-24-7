...
def callfn(sql):
     ...
     #      sql = "select customer_mobile from customer_details where business_id="+business_id+" and customer_appointment_date between  '"+date_from+"' and '"+date_to+"' and customer_current_status in ('checkedout')"
           ...
           con.close()         
           values = json.dumps({"Message":"SMS Sent Sucessfully"},indent =2)       
           return(values)    
def sendsms(request):
       if request.args.get('business_id') and request.args.get('message') and request.args.get('date_from') and request.args.get('date_to') and request.args.get('status'):
           
           date_from = request.args['date_from']
           date_to = request.args['date_to']
           business_id = request.args['business_id']
           status = request.args['status']
           sql = "select customer_mobile from customer_details where business_id="+business_id+" and customer_appointment_date between  '"+date_from+"' and '"+date_to+"' and customer_current_status in ('"+status+"') "
           return(callfn(sql))
       elif request.args.get('business_id') and request.args.get('message') and request.args.get('date_from') and request.args.get('date_to'):
